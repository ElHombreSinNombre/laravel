...
class VisitantesEntranHoyDataTable extends DataTable
{
    public function dataTable($query)
    {
        $query->whereYear('f_entrada',Carbon::now()->year)->whereMonth('f_salida',Carbon::now()->month)->whereDay('f_salida',Carbon::now()->day)->get();
        $hoyEntran = new EloquentDataTable($query);
        return $hoyEntran
            //Columna estado
            ->addColumn('estado', function ($hoyEntran) {
                return $hoyEntran->previsita ?  '<a class="text-danger" data-toggle="tooltip" data-placement="top" title="Cambiar estado" href="' . route('dashboard.visitas.estado', [$hoyEntran->id]) . '"><strong>Salida</strong></a>': '<a class="text-success" data-toggle="tooltip" data-placement="top" title="Cambiar estado" href="' . route('dashboard.visitas.estado', [$hoyEntran->id]) . '"><strong>Entrada</strong></a>';
            })->rawColumns(['estado'])
    ...
    public function html()
    {
        return $this->builder()
                    ...
                    ->parameters([
                        'order'      => [[0, 'desc']],
                        'dom'        => "<'row'<'col-xs-12'<'col-xs-6'l>>><'row'<'col-xs-12't>><'row'<'col-xs-12'<'col-xs-6'i><'col-xs-6'p>>>",
                        'processing' => true,
                        'serverSide' => true,
                        'responsive' => true,
                        'searching'  => false,
                    ...
                    ->ajax([
                        'url'  => route('dashboard.visitas.hoyEntran'),
                    ]);
    ...
    {
        return 'Listado de visitas que entran ' . Carbon:: now()->format('d/m/Y H:i:s');
    }
