...
class VisitantesSalenHoyDataTable extends DataTable
{
    public function dataTable($query)
    {
        $query->whereYear('f_salida',Carbon::now()->year)->whereMonth('f_salida',Carbon::now()->month)->whereDay('f_salida',Carbon::now()->day)->get();
        $hoySalen = new EloquentDataTable($query);
        return $hoySalen
            //Columna estado
            ->addColumn('estado', function ($hoySalen) {
                return $hoySalen->previsita ?  '<a class="text-danger" data-toggle="tooltip" data-placement="top" title="Cambiar estado" href="' . route('dashboard.visitas.estado', [$hoySalen->id]) . '"><strong>Salida</strong></a>': '<a class="text-success" data-toggle="tooltip" data-placement="top" title="Cambiar estado" href="' . route('dashboard.visitas.estado', [$hoySalen->id]) . '"><strong>Entrada</strong></a>';
            })->rawColumns(['estado'])
    ...
    public function html()
    {
        return $this->builder()
                    ...
                    ->ajax([
                        'url'  => route('dashboard.visitas.hoySalen'),
                    ]);
    ...
    {
        return 'Listado de visitas que salen el ' . Carbon:: now()->format('d/m/Y H:i:s');
    }
