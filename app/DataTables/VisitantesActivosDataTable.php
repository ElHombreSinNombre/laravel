...
class VisitantesActivosDataTable extends DataTable
{
    public function dataTable($query)
    {
        $query->whereNotNull('f_entrada')->whereNull('f_salida')->where('previsita',0);
        $activos = new EloquentDataTable($query);
        return $activos
            //Columna estado
            ->addColumn('estado', function ($activos) {
                return $activos->previsita ? '<a class="text-success" data-toggle="tooltip" data-placement="top" title="Cambiar estado" href="' . route('dashboard.visitas.estado', [$activos->id]) . '"><strong>Entrada</strong></a>' :  '<a class="text-danger" data-toggle="tooltip" data-placement="top" title="Cambiar estado" href="' . route('dashboard.visitas.estado', [$activos->id]) . '"><strong>Salida</strong></a>';
            })->rawColumns(['estado'])
    ...
    public function html()
    {
        return $this->builder()
                    ...
                    ->ajax([
                        'url'  => route('dashboard.visitas.activos'),
                    ]);
    ...
    {
        return 'Listado de visitas activas del ' . Carbon:: now()->format('d/m/Y H:i:s');
    }
